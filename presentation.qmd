...
## Постановка задачи: MPPI и его пробел
...
$$
\dot{\mathbf{v}} = \mathbf{M}^{-1}\big[\bar{\pmb{\Phi}}\,\mathbf{n} + \bar{\pmb{\Phi}}\,\mathbf{B}\,\mathbf{u} + \pmb{\beta}\big]
$$
:::
...
$$
\mathbf{W} = \mathbf{A}_\text{task}\mathbf{M}^{-1}\bar{\pmb{\Phi}}\mathbf{B}, \qquad
\pmb{\varepsilon} = \mathbf{b}_\text{task} - \mathbf{A}_\text{task}\mathbf{M}^{-1}(\bar{\pmb{\Phi}}\mathbf{n} + \pmb{\beta})
$$

::: {.fragment .fade-up}
* Решение минимальной нормы --- проекция на ядро:
$$
...
* Псевдообратная с регуляризацией Тихонова [@tikhonov1977] --- устойчивость вблизи сингулярностей
:::

## Стабилизация Баумгарта

* Механизм работает на уровне ускорений ($\ddot{h} = 0$), но численное интегрирование вызывает **дрейф** на уровне положений

::: {.fragment .fade-up}
* Баумгарт [@baumgarte1972]: заменить $\ddot{h}=0$ критически демпфированной формой
$$
...
3. Ограниченное ускорение: $\;\dot{\mathbf{v}} \gets \mathbf{M}^{-1}[\bar{\pmb{\Phi}}(\mathbf{B}\mathbf{u}_c + \mathbf{n}) + \pmb{\beta}]$
...
::: {.fragment .fade-up}
[Научная новизна:]{style="color:green;"} впервые принцип наименьшего принуждения Гаусса соединён с предиктивным управлением на основе семплирования.
:::
